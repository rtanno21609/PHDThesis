...
Firstly, you would be interested in knowing how well your model performs in a new environment, but you . \textit{Predictive uncertainty} refers to the degree of ambiguity (or confidence) in the model's prediction on a given input. For example, error bounds are a common measure of predictive uncertainty. This can be used to assess the reliability of prediction for the particular data instance at hands and act accordingly; if the model encouters input examples with high predictive uncertainty, then we should not trust the predictions. 

Secondly, you would also want to understand the sources of predictive uncertainty. By identifying and quantifying 
 


%At a very fundamental level, the first level in understanding the uncertainties affecting some system is to identify the sources of uncertainty

% Create a figure to describe other things. 
